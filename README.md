...
- It is now possible to force the plugin into using generic tools. Because the plugin is already using all avaiable settings fields i combined this setting with the address field. If you add '#forcegeneric' behind the address it will skip the detection of chipset specific tools (eg 192.168.0.1#forcegeneric).
